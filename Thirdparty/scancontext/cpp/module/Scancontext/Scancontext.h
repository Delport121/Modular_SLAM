...
public:
    ...
    const double PC_MAX_RADIUS = 8.0; // 80 meter max in the original paper (IROS 18)
    ...
    const double SC_DIST_THRES = 0.15; // empirically 0.1-0.2 is fine (rare false-alarms) for 20x60 polar context (but for 0.15 <, DCS or ICP fit score check (e.g., in LeGO-LOAM) should be required for robustness)
